...
pipeline on any violation:
    ...
    extractions.jsonl
      ...
      -> synthesize  (fill zero-represented dx; $0 no-op at 100 scale) -> synthesized.jsonl
...
def main() -> None:
    ...
    # 4) synthesize (paid, cache-first; no-op when no category is short)
    ...
    _write(synthesize.SYNTHESIZED_PATH, augmented)
    print(f"synthesize: nothing under-represented — $0 no-op ({len(augmented)} records)")

    # 5) DQ gate (fails the pipeline)
    report = dq.validate_records(augmented)
    dq.REPORT_PATH.parent.mkdir(parents=True, exist_ok=True)
    dq.REPORT_PATH.write_text(json.dumps(report, indent=2))
    ...
    if not report["passed"]:
        raise SystemExit(f"DQ GATE FAILED — see {dq.REPORT_PATH}")
    print(f"DQ gate PASSED across {report['n_records']} curated records.")
